...
def main():
    ...
    parser.add_argument('--output-dir', type=str, default=None, help='指定输出文件夹（相对于 Papers 目录），不指定则按 domain 自动推断 / Custom output folder (relative to Papers dir)')
    ...
    if args.output_dir:
        # 用户指定了输出文件夹，直接使用
        output_dir = args.output_dir.strip('/\\').replace('..', '')
        if not output_dir:
