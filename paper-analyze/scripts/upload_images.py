...
Reads OSS config from PicGo's data.json:
...
def load_picgo_config():
    """Load OSS config from PicGo's data.json."""
    appdata = os.environ.get("APPDATA", "")
    picgo_config_path = os.path.join(appdata, "picgo", "data.json")

    if not os.path.exists(picgo_config_path):
        print(f"ERROR: PicGo config not found at {picgo_config_path}", file=sys.stderr)
        sys.exit(1)

    with open(picgo_config_path, "r", encoding="utf-8") as f:
        config = json.load(f)

    aliyun = config.get("picBed", {}).get("aliyun", {})
    if not aliyun.get("accessKeyId"):
        print("ERROR: Aliyun OSS not configured in PicGo", file=sys.stderr)
        sys.exit(1)
